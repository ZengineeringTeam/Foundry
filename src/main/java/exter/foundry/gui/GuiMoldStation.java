...
public class GuiMoldStation extends GuiFoundry
{
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int x, int y)
    {
        ...
        if (te_ms.hasBlock())
        {
            drawTexturedModalRect(window_x + BLOCK_X, window_y + BLOCK_Y, BLOCK_OVERLAY_X, BLOCK_OVERLAY_Y, BLOCK_SIZE,
                    BLOCK_SIZE);
            for (int i = 0; i < 36; i++)
            {
                int gx = i % 6;
                ...
                if (sv > 0)
                {
                    drawTexturedModalRect(window_x + GRID_X + gx * GRID_SLOT_SIZE,
                            window_y + GRID_Y + gy * GRID_SLOT_SIZE, GRID_OVERLAY_X,
                            GRID_OVERLAY_Y + (sv - 1) * GRID_SLOT_SIZE, GRID_SLOT_SIZE, GRID_SLOT_SIZE);
                }
            }
        }
    }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
    {
        ...
        if (processingState == -1 && te_ms.hasBlock()
                && isPointInRegion(GRID_X, GRID_Y, GRID_SIZE - 1, GRID_SIZE - 1, mouseX, mouseY) && mouseButton >= 0
                && mouseButton <= 1)
        {
