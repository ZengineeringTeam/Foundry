...
public class GuiMaterialRouter extends GuiFoundry
{
        ...
        if (button.id < 6)
        {
            te_router.getRoutes()
                    .add(new TileEntityMaterialRouter.Route(
                            ...
                            type_slots.get(te_router.gui_type_selected).getName(), EnumFacing.VALUES[button.id]));
            te_router.syncRoutes();
        }
    ...
    protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3)
    {
        ...
        if (selected_material >= material_scroll && selected_material < material_scroll + 8)
        {
            ...
            drawTexturedModalRect(window_x + 111 + 17 * (index % 4), window_y + 24 + 17 * (index / 4), 200, 193, 16,
                    16);
        }
        ...
        if (selected_type >= type_scroll && selected_type < type_scroll + 8)
        {
            ...
            drawTexturedModalRect(window_x + 111 + 17 * (index % 4), window_y + 70 + 17 * (index / 4), 200, 193, 16,
                    16);
        }
    ...
    protected void drawGuiContainerForegroundLayer(int mouse_x, int mouse_y)
    {
        ...
        fontRenderer.drawString("Types(" + (te_router.gui_type_scroll / 8 + 1) + "/"
                + (int) Math.ceil((double) type_slots.size() / 8) + "):", 111, 60, 0x404040);
    ...
    public void initGui()
    {
        ...
        for (i = 0; i < 6; i++)
        {
            route_buttons[i] = new GuiButtonFoundry(i, window_x + 119 + i % 3 * 18, window_y + 108 + i / 3 * 18, 16, 16,
                    GUI_TEXTURE, 200, 177, 216, 177).setIconTexture(201, i * 16 + 1, 14, 14);
            buttonList.add(route_buttons[i]);
        }
        material_scroll_left = new GuiButtonFoundry(6, window_x + 96, window_y + 28, 12, 25, GUI_TEXTURE, 200, 99, 212,
                99);
        buttonList.add(material_scroll_left);
        material_scroll_right = new GuiButtonFoundry(7, window_x + 181, window_y + 28, 12, 25, GUI_TEXTURE, 200, 124,
                212, 124);
        ...
        type_scroll_right = new GuiButtonFoundry(9, window_x + 181, window_y + 74, 12, 25, GUI_TEXTURE, 200, 124, 212,
                124);
        buttonList.add(type_scroll_right);
        route_scroll_up = new GuiButtonFoundry(10, window_x + 47, window_y + 24, 25, 12, GUI_TEXTURE, 200, 149, 225,
                149);
        buttonList.add(route_scroll_up);
        route_scroll_down = new GuiButtonFoundry(11, window_x + 47, window_y + 119, 25, 12, GUI_TEXTURE, 200, 161, 225,
                161);
