...
public class TileEntityRefractorySpout extends TileEntityFoundry
{
    ...
    protected void updateServer()
    {
        ...
        if (--next_move == 0)
        {
            ...
            if (world.getBlockState(getPos())
                    .getValue(BlockFoundrySidedMachine.STATE) == BlockFoundrySidedMachine.EnumMachineState.ON)
            {
                ...
                if (drained != null)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (dest != null
                                && dest.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, EnumFacing.UP))
                        {
                            IFluidHandler hdest = dest.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY,
                                    EnumFacing.UP);
                            if (drained != null)
                            {
