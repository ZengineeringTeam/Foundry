...
public class TileEntityMoldStation extends TileEntityFoundry implements IExoflameHeatable
{
    ...
    private boolean canOutput(ItemStack output, int slot)
    {
        ...
        if (inv_output.isEmpty())
            return true;
        return inv_output.isItemEqual(output)
    ...
    private boolean canRecipeOutput()
    {
        ...
        return canOutput(output, SLOT_OUTPUT)
                && (slot_clay.isEmpty() || slot_clay.getCount() + clay_amount <= slot_clay.getMaxStackSize());
    }
