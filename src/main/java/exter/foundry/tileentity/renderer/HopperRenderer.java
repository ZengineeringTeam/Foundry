...
public class HopperRenderer extends TileEntitySpecialRenderer<TileEntityRefractoryHopper>
{
    ...
    public void render(TileEntityRefractoryHopper te, double x, double y, double z, float partialTicks, int destroyStage, float a)
    {
        ...
        if (fluid != null && fluid.amount > 80 && pos.getY() < world.getHeight()
                && !world.getBlockState(pos).doesSideBlockRendering(world, pos, EnumFacing.DOWN))
        {
            GL11.glPushMatrix();
            ...
            tessellator.pos(1.995 / 16, fluid_z / 16, 14.005 / 16).color(red, green, blue, alpha).tex(min_u, max_v)
                    .lightmap(l1, l2).endVertex();
            tessellator.pos(14.005 / 16, fluid_z / 16, 14.005 / 16).color(red, green, blue, alpha).tex(max_u, max_v)
                    .lightmap(l1, l2).endVertex();
            tessellator.pos(14.005 / 16, fluid_z / 16, 1.995 / 16).color(red, green, blue, alpha).tex(max_u, min_v)
                    .lightmap(l1, l2).endVertex();
            tessellator.pos(1.995 / 16, fluid_z / 16, 1.995 / 16).color(red, green, blue, alpha).tex(min_u, min_v)
                    .lightmap(l1, l2).endVertex();
