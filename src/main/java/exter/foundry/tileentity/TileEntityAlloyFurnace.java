...
public class TileEntityAlloyFurnace extends TileEntityFoundry implements ISidedInventory, IExoflameHeatable
{
    ...
    public TileEntityAlloyFurnace()
    {
        ...
        item_handler_fuel = new ItemHandlerFuel(this, getSizeInventory(), IH_SLOTS_INPUT_FUEL, IH_SLOTS_OUTPUT_FUEL,
                IH_SLOTS_FUEL);
    }
    ...
    private boolean canOutput(IAlloyFurnaceRecipe recipe)
    {
        ...
        return inv_output.isEmpty() || (inv_output.isItemEqual(output)
                && inv_output.getCount() - output.getCount() <= inv_output.getMaxStackSize());
    }
    ...
    public boolean canSmelt()
    {
        ...
        {
            IAlloyFurnaceRecipe recipe = AlloyFurnaceRecipeManager.INSTANCE.findRecipe(getStackInSlot(SLOT_INPUT_A),
                    getStackInSlot(SLOT_INPUT_B));
            if (recipe == null)
            {
                recipe = AlloyFurnaceRecipeManager.INSTANCE.findRecipe(getStackInSlot(SLOT_INPUT_B),
                        getStackInSlot(SLOT_INPUT_A));
            }
            if (recipe == null)
            {
                return false;
            ...
            if (!inv_output.isEmpty() && (!inv_output.isItemEqual(output)
                    || inv_output.getCount() - output.getCount() > inv_output.getMaxStackSize()))
            {
                return false;
            }
    ...
    public void readFromNBT(NBTTagCompound tag)
    {
        ...
        {
            ((BlockAlloyFurnace) getBlockType()).setMachineState(world, getPos(), world.getBlockState(getPos()),
                    burn_time > 0);
        }
    }
    ...
    protected void updateServer()
    {
        ...
        if (!getStackInSlot(SLOT_INPUT_A).isEmpty() && !getStackInSlot(SLOT_INPUT_B).isEmpty())
        {
            recipe = AlloyFurnaceRecipeManager.INSTANCE.findRecipe(getStackInSlot(SLOT_INPUT_A),
                    getStackInSlot(SLOT_INPUT_B));
            if (recipe == null)
            {
                recipe = AlloyFurnaceRecipeManager.INSTANCE.findRecipe(getStackInSlot(SLOT_INPUT_B),
        ...
        if (burn_time == 0 && recipe != null && canOutput(recipe))
        {
            ...
            if (burn_time > 0)
            {
                if (!getStackInSlot(SLOT_FUEL).isEmpty())
                {
                    ...
                    {
                        setStackInSlot(SLOT_FUEL,
                                getStackInSlot(SLOT_FUEL).getItem().getContainerItem(getStackInSlot(SLOT_FUEL)));
                    }
