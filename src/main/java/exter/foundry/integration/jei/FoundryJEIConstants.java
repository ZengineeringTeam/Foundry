package exter.foundry.integration.jei;

import exter.foundry.Foundry;

public class FoundryJEIConstants
{

    public static final String MELT_UID = Foundry.MODID + ".melting";
    public static final String CAST_UID = Foundry.MODID + ".casting";
    public static final String INF_UID = Foundry.MODID + ".infuser";
    public static final String MOLD_UID = Foundry.MODID + ".mold";
    public static final String AM_UID = Foundry.MODID + ".alloymixer";
    public static final String AC_UID = Foundry.MODID + ".alloyingcrucible";

}
