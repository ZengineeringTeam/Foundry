    ...
    public void onAfterPostInit()
    {
            ...
            for (ItemStack stack : recipe.input.getInputs())
            {
                if (!stack.isEmpty() && MeltingRecipeManager.INSTANCE.findRecipe(stack) == null)
                {
                    ...
                    if (mapped != null)
                    {
                        ...
                        {
                            mapped_liquid = new FluidStack(LiquidMetalRegistry.INSTANCE.getFluid(mapped),
                                    result.amount);
                        }
                        else
                        {
                            mapped_liquid = new FluidStack(LiquidMetalRegistry.INSTANCE.getFluid(mapped),
                                    FoundryMiscUtils.divCeil(result.amount * FoundryAPI.FLUID_AMOUNT_INGOT,
                                            TICON_INGOT_AMOUNT));
                        }
            ...
            if (!icasting.consumesCast())
            {
                if (icasting instanceof CastingRecipe)
                {
                    ...
                    if (casting.cast != null && !casting.consumesCast() && !casting.getResult().isEmpty())
                    {
                        ...
                        {
                            mapped_liquid = new FluidStack(LiquidMetalRegistry.INSTANCE.getFluid(mapped),
                                    FoundryMiscUtils.divCeil(casting.getFluid().amount * FoundryAPI.FLUID_AMOUNT_INGOT,
                                            TICON_INGOT_AMOUNT));
                        }
                        for (ItemStack cast : casting.cast.getInputs())
                        {
                            ...
                            if (mapped_liquid != null)
                            {
                                if (mapped_liquid.amount <= 6000)
                                {
                                    CastingRecipeManager.INSTANCE.addRecipe(new ItemStackMatcher(casting.getResult()),
                                            mapped_liquid, cast, null);
                                }
                            }
                            if (casting.getFluid().amount <= 6000)
                            {
                                CastingRecipeManager.INSTANCE.addRecipe(new ItemStackMatcher(casting.getResult()),
                                        casting.getFluid(), cast, null);
                            }
                        }
                    }
                }
        ...
        for (ICastingRecipe icasting : TinkerRegistry.getAllBasinCastingRecipes())
        {
            if (icasting instanceof CastingRecipe)
            {
                ...
                {
                    CastingRecipeManager.INSTANCE.addRecipe(new ItemStackMatcher(casting.getResult()), fluid,
                            block_mold, null);
                }
            }
        }
        ...
        for (ICastingRecipe icasting : TinkerRegistry.getAllTableCastingRecipes())
        {
            if (icasting instanceof CastingRecipe)
            {
                ...
                FluidStack mapped_liquid = new FluidStack(fluid,
                        mapped.equals("Glass") ? casting.getFluid().amount
                                : FoundryMiscUtils.divCeil(casting.getFluid().amount * FoundryAPI.FLUID_AMOUNT_INGOT,
                                        TICON_INGOT_AMOUNT));
                CastingRecipe recipe = new CastingRecipe(casting.getResult(), casting.cast, mapped_liquid,
                        casting.consumesCast(), casting.switchOutputs());
                recipes.add(recipe);
            }
        }
            ...
            if (icasting instanceof CastingRecipe)
            {
                ...
                FluidStack mapped_liquid = new FluidStack(fluid,
                        mapped.equals("Glass") ? casting.getFluid().amount
                                : FoundryMiscUtils.divCeil(casting.getFluid().amount * FoundryAPI.FLUID_AMOUNT_INGOT,
                                        TICON_INGOT_AMOUNT));
                CastingRecipe recipe = new CastingRecipe(casting.getResult(), null, mapped_liquid,
                        casting.consumesCast(), casting.switchOutputs());
                recipes.add(recipe);
            }
        }
