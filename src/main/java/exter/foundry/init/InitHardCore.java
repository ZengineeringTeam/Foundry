...
public class InitHardCore
{
    ...
    static public void init()
    {
        ...
        { // Remove furnace recipes that outputs ingots (except the ones in the keep list or can't be melted).
            Iterator<Map.Entry<ItemStack, ItemStack>> iter = FurnaceRecipes.instance().getSmeltingList().entrySet()
                    .iterator();
            while (iter.hasNext())
            {
                ...
                for (String name : ore_names)
                {
                    if (name.startsWith("ingot") && !FoundryConfig.hardcore_furnace_keep_ingots.contains(name)
                            && MeltingRecipeManager.INSTANCE.findRecipe(recipe.getValue()) != null)
                    {
                        iter.remove();
                        break;
                    }
