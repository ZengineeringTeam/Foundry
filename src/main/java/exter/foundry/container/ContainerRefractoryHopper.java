...
public class ContainerRefractoryHopper extends Container
{
    ...
    public ContainerRefractoryHopper(TileEntityRefractoryHopper hopper, EntityPlayer player)
    {
        ...
        addSlotToContainer(
                new SlotFluidContainer(te_hopper, TileEntityRefractoryHopper.INVENTORY_CONTAINER_DRAIN, 94, 20));
        addSlotToContainer(
                new SlotFluidContainer(te_hopper, TileEntityRefractoryHopper.INVENTORY_CONTAINER_FILL, 94, 48));
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            if (slot_index >= SLOTS_INVENTORY && slot_index <= SLOTS_HOTBAR + 9)
            {
                if (stack.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, null))
                {
                    ...
                    if (f != null && mergeItemStack(stack, TileEntityRefractoryHopper.INVENTORY_CONTAINER_DRAIN,
                            TileEntityRefractoryHopper.INVENTORY_CONTAINER_DRAIN + 1, false))
                    {
                        return ItemStack.EMPTY;
                    }
                    else if (mergeItemStack(stack, TileEntityRefractoryHopper.INVENTORY_CONTAINER_FILL,
                            TileEntityRefractoryHopper.INVENTORY_CONTAINER_FILL + 1, false))
                    {
                        return ItemStack.EMPTY;
                    }
                }
